*"* Query provider for custom entity ZSD_PMS_GEO.
...
*"* blocks). Maps ls_dash-geo (state rows) - see ZSD_PMS_GEO.ddls.asddls's
*"* header for why Unit dots are a separate entity instead of a
*"* "Granularity" slice of this one.
CLASS zcl_pms_geo_qry DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_pms_geo_qry IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    DATA lt_out TYPE STANDARD TABLE OF zsd_pms_geo.
    lt_out = VALUE #( FOR ls IN ls_dash-geo (
                        regio        = ls-regio
