*"* Query provider for custom entity ZSD_PMS_UNIT_DOTS.
...
*"* blocks). Maps ls_dash-unit_dots (OD-3/OD-5 - value-weighted centroid
*"* per Unit, plants with no coordinates already excluded upstream).
CLASS zcl_pms_unit_dots_qry DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_pms_unit_dots_qry IMPLEMENTATION.
