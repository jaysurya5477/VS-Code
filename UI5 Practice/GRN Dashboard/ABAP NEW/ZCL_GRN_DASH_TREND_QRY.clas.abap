*"* Query provider for custom entity ZMM_GRN_DASH_TREND.
...
CLASS zcl_grn_dash_trend_qry DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_grn_dash_trend_qry IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    DATA lt_out TYPE STANDARD TABLE OF zmm_grn_dash_trend.
    lt_out = VALUE #( FOR ls IN ls_dash-trend (
                        period = ls-period
                        qty101 = ls-qty101
                        qty102 = ls-qty102
                        qtyz22 = ls-qtyz22
    ...
    " $orderby the front end ever sends (ZCL_GRN_DASH_QUERY: SORT lt_trend BY period
    " ASCENDING), so there is nothing further to apply. RAP still requires the provider to
