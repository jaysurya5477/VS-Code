*"* Query provider for custom entity ZMM_GRN_DASH_MAT_REJ_WORST10.
...
CLASS zcl_grn_dash_mat_rej_w10_qry DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_grn_dash_mat_rej_w10_qry IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    DATA lt_out TYPE STANDARD TABLE OF zmm_grn_dash_mat_rej_worst10.
    lt_out = VALUE #( FOR ls IN ls_dash-material_rej_worst10 (
                        material     = ls-matnr
                        materialname = ls-txz01
                        rejqty       = ls-rej_qty
                        rejpct       = ls-rej_pct ) ).

    " ---- $orderby ----
    " get_dashboard_data() already returns this result set pre-sorted to match the one
    " $orderby the front end ever sends (ZCL_GRN_DASH_QUERY: SORT rt_mr BY rej_pct
