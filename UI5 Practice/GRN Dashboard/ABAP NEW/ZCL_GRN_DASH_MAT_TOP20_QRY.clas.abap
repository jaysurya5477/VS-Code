*"* Query provider for custom entity ZMM_GRN_DASH_MATERIAL_TOP20.
...
CLASS zcl_grn_dash_mat_top20_qry DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_grn_dash_mat_top20_qry IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    DATA lt_out TYPE STANDARD TABLE OF zmm_grn_dash_material_top20.
    lt_out = VALUE #( FOR ls IN ls_dash-material_top20 (
                        material     = ls-matnr
                        materialname = ls-txz01
                        value        = ls-value
                        grandtotal   = ls-grand_total ) ).

    " ---- $orderby ----
    " get_dashboard_data() already returns this result set pre-sorted to match the one
    " $orderby the front end ever sends (ZCL_GRN_DASH_QUERY: SORT lt_mat BY value
