*"* Query provider for custom entity ZMM_GRN_DASH_VENDOR_SCORECARD.
...
CLASS zcl_grn_dash_vendor_score_qry DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_grn_dash_vendor_score_qry IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    DATA lt_out TYPE STANDARD TABLE OF zmm_grn_dash_vendor_scorecard.
    lt_out = VALUE #( FOR ls IN ls_dash-vendor_scorecard (
                        vendor     = ls-lifnr
                        vendorname = ls-name1
                        qty        = ls-qty
                        netqty     = ls-net_qty
    ...
    " $orderby the front end ever sends (ZCL_GRN_DASH_QUERY: SORT rs_result-vendor_scorecard
    " BY score DESCENDING), so there is nothing further to apply. RAP still requires the
