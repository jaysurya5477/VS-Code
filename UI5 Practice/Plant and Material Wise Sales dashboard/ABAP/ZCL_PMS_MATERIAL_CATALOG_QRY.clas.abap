*"* Query provider for custom entity ZSD_PMS_MATERIAL_CATALOG.
...
*"* blocks). Maps ls_dash-material_catalog - get_material_catalog( )
*"* returns every distinct material in scope, uncapped (see that method's
*"* own header for why this must not reuse ls_dash-material_top20).
*"*
*"* Same known gap as ZCL_PMS_MATERIAL_QRY: P_Scheme does not restrict this
*"* entity - category lives only on the GL-grain fact, not on VBRP/VBRK.
CLASS zcl_pms_material_catalog_qry DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_pms_material_catalog_qry IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    DATA lt_out TYPE STANDARD TABLE OF zsd_pms_material_catalog.
    lt_out = VALUE #( FOR ls IN ls_dash-material_catalog (
                        matnr = ls-matnr
                        arktx = ls-arktx
                        bismt = ls-bismt ) ).
