*"* Query provider for custom entity ZSD_PMS_TREND.
...
*"* blocks). get_trend( ) already returns its result pre-sorted by Period
*"* ascending, so there is nothing to re-sort here either - see
*"* get_sort_elements( )'s own comment below for why it's still called.
CLASS zcl_pms_trend_qry DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_pms_trend_qry IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    DATA lt_out TYPE STANDARD TABLE OF zsd_pms_trend.
    lt_out = VALUE #( FOR ls IN ls_dash-trend (
                        period   = ls-period
                        netvalue = ls-net_value ) ).

    " get_trend( ) already sorts BY period ASCENDING - this call doesn't
    " re-sort, it only satisfies RAP's requirement to consume $orderby
    " when the caller sends one (see ZCL_PMS_KPI_QRY's comment).
