*"* Query provider for custom entity ZMM_GRN_DASH_DOCTYPE_RANKED.
...
CLASS zcl_grn_dash_doctype_rank_qry DEFINITION
  PUBLIC
...
CLASS zcl_grn_dash_doctype_rank_qry IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    DATA lt_out TYPE STANDARD TABLE OF zmm_grn_dash_doctype_ranked.
    lt_out = VALUE #( FOR ls IN ls_dash-doctype_ranked (
                        doctype     = ls-bsart
                        doctypename = ls-batxt
                        value       = ls-value ) ).

    " ---- $orderby ----
    " get_dashboard_data() already returns this result set pre-sorted to match the one
    " $orderby the front end ever sends (ZCL_GRN_DASH_QUERY: SORT lt_dt BY value
