*"* Query provider for custom entity ZMM_GRN_DASH_PLANT_TOP10.
...
CLASS zcl_grn_dash_plant_top10_qry DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_grn_dash_plant_top10_qry IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    DATA lt_out TYPE STANDARD TABLE OF zmm_grn_dash_plant_top10.
    lt_out = VALUE #( FOR ls IN ls_dash-plant_top10 (
                        plant       = ls-werks
                        totalqty    = ls-total_qty
                        acceptedpct = ls-accepted_pct
                        rejectedpct = ls-rejected_pct
                        samplepct   = ls-sample_pct
                        inspectpct  = ls-inspect_pct ) ).

    " ---- $orderby ----
    " get_dashboard_data() already returns this result set pre-sorted to match the one
    " $orderby the front end ever sends (ZCL_GRN_DASH_QUERY: SORT rt_plant BY total_qty
    " DESCENDING), so there is nothing further to apply. RAP still requires the provider to
    " consume get_sort_elements() whenever the request carries $orderby, or it rejects the
    " whole read with RAP_RUNTIME/004 ("Query not fully covered by implementation") even
    " though the data would have come back correctly ordered regardless.
