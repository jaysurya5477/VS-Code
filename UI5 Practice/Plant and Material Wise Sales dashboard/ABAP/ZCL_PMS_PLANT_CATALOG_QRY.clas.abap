*"* Query provider for custom entity ZSD_PMS_PLANT_CATALOG.
...
*"* blocks). Maps ls_dash-plant_catalog - get_plant_catalog( ) returns
*"* every distinct plant in scope, uncapped (see that method's own header
*"* for why this must not reuse ls_dash-plant_top20).
CLASS zcl_pms_plant_catalog_qry DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_pms_plant_catalog_qry IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    DATA lt_out TYPE STANDARD TABLE OF zsd_pms_plant_catalog.
    lt_out = VALUE #( FOR ls IN ls_dash-plant_catalog (
                        werks = ls-werks
                        city  = ls-city ) ).
