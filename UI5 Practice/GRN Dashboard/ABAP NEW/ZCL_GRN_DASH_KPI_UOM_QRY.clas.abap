*"* Query provider for custom entity ZMM_GRN_DASH_KPI_BY_UOM.
...
*"* every UoM (see ZMM_GRN_DASH_KPI_BY_UOM's header comment); the
*"* dashboard's UoM picker never scopes this one.
CLASS zcl_grn_dash_kpi_uom_qry DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_grn_dash_kpi_uom_qry IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    DATA lt_out TYPE STANDARD TABLE OF zmm_grn_dash_kpi_by_uom.
    lt_out = VALUE #( FOR ls IN ls_dash-kpi_by_uom (
                        uom        = ls-meins
                        grossqty   = ls-gross_qty
                        grossvalue = ls-gross_value
                        netqty     = ls-net_qty
                        netvalue   = ls-net_value ) ).

    " ---- $orderby ----
    " get_dashboard_data() already returns this result set pre-sorted (ZCL_GRN_DASH_QUERY:
    " SORT lt_kpi BY gross_qty DESCENDING in get_kpi_by_uom), so there is nothing further to
    " apply. RAP still requires the provider to consume get_sort_elements() whenever the
    " request carries $orderby, or it rejects the whole read with RAP_RUNTIME/004 ("Query not
    " fully covered by implementation") even though the data would have come back correctly
    " ordered anyway.
